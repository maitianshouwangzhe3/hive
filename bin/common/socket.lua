...
local tab_isempty = function(tab)
    return next(tab) == nil
end

local tab_isarray = function(tab)
...
local function ev_base_handler(fd, readable, writable, errevent)
    ...
    if not ok then
        log_err("ev_base_handler error: %s", err)
        close(fd)
    end
end
...
function _M.listen(endpoint, on_accept)
    ...
    log_debug("listen: %s:%s", host, port)
    port = tonumber(port)
...
function _M.sync_write(fd, buf, len)
    return anet.sync_send(fd, buf, len)
end

function _M.async_write(fd, buf, len)
    local s = assert(socket_pool[fd])
    local ok = s.wbuffer:async_write_binary(buf, len)
    if not ok then
        s.writable = true
        ae.enable(aefd, fd, true, true)
        s.is_resume = false
