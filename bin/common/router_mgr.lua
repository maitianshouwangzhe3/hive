...
function setup(service_name)
    ...
    hive.group_id = group_id;
    hive.index = tonumber(args.index) or 1;
    hive.id = make_service_id(group_id, hive.index);
...
function connect(node)
    ...
    socket.on_connect = function(res)
        ...
        log_info("connect router: %s:%s", node.ip, node.port);
        node.alive = true;
        node.alive_time = hive.now;
        socket.call("register", hive.group_id, hive.index);
        switch_master();
    end
...
function switch_master()
    ...
    master = nil;
    -- log_debug("switch router: %s", #candidates);
