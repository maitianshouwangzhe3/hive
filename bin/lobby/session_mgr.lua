...
function on_accept(ss)
    ...
    ss.on_pb_recv = function(token, data, data_len)
        return rpc_mgr.dispatch(token, data, data_len)
    end
...
function update(frame)
	if frame % 100 == 0 then
		_G.call_gateway_all("sync_payload", hive.id, session_count, listen_ip, listen_port);
	end
end
