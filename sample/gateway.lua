...
if not hive.init_flag then
    ...
    hive.print = log_info;
    _G.print = log_debug;
    _G.socket_mgr = hive.create_socket_mgr(args.connections or 1024);
...
function find_best_lobby()
	...
	for id, node in pairs(lobby_list) do
        log_debug("node.time=%s, now=%s", node.time, now)
...
function c2s.login_req(ss)
    log_info("login_req, token=%s", ss.token);
	local node = find_best_lobby();
	if not node then
        log_debug("lobby not open")
		ss.call("login_res", "not-open", hotfix);
		return;
	end
	node.payload = node.payload + 1;
    log_debug("login to lobby, id=%s, payload=%s, url=%s:%s", service_id2name(node.id), node.payload, node.ip, node.port)
	-- ss.call("login_res", "ok", hotfix, node.ip, node.port);
    local stream = lobby_stream[node.id];
    if stream then
        stream.call("login_res", hotfix, node.ip, node.port);
    end
end
